...
public class GlassesEventPacket extends Packet<GlassesEventPacket, IMessage> {
	...
	Location UUID;
	String player;
	int x, y, mb, width, height, scaleFactor;
	...
	public GlassesEventPacket(EventType eventType, Location UUID, EntityPlayer player, int x, int y, int scaleFactor) {
		this(eventType, UUID, player);
		this.x = x;
		this.y = y;
	...
	protected void read() throws IOException {
		...
		if (EventType.UNEQUIPED_GLASSES == eventType) {
			return;
		}
		...
		if (EventType.GLASSES_SCREEN_SIZE == eventType) {
			this.x = readInt();
			this.y = readInt();
			this.scaleFactor = readInt();
		}
	}
	...
	protected IMessage executeOnServer() {
		...
		switch (eventType) {
			...
			case GLASSES_SCREEN_SIZE:
				...
				terminal = UUID.getTerminal();
				if (playerMP != null && terminal != null) {
					terminal.sendChangeSizeEvent(eventType.name(), playerMP.getName(), x, y, scaleFactor);
				}
				break;
			default:
				break;
